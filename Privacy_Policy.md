...
If you subscribe to a paid plan, payment is handled by a third-party payment processor. **We do not receive or store your full card number, CVC, or bank credentials** — those go directly to the processor. We retain only what we need to administer your subscription: which Discord server the plan applies to, the plan tier, its status and renewal date, and the processor's transaction reference. We may also receive limited billing details from the processor, such as the last four digits of a card, its expiry, and the billing country, for invoicing and fraud prevention. Where required by tax or accounting law, invoice records are kept for the statutory retention period.
...
- **Payment processors**, if you subscribe to a paid plan, to take payment and handle billing, refunds, and chargebacks — see Section 1(f). They act as independent controllers of the card data you give them, under their own privacy policies.
