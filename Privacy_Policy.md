...
- A signed session cookie/token, used to keep you logged in to the verification page during a single verification attempt. This cookie is not stored on our servers вЂ” it lives only in your browser and is cryptographically signed so we can detect tampering.
...
- Image attachments, when a message is flagged as a likely scam вЂ” the image is copied to a private moderation channel as evidence and automatically deleted after a configurable retention period (24 hours by default)
...
**Confirmed scam reports and false-positive flags are forwarded to ohShield's operator.** When a `!scam` report results in a ban, a copy of the alert (including the reported message's text, the reported user's ID, and the server's name/ID) is forwarded to a private, operator-controlled Discord channel, so the operator can track detection accuracy across servers. Separately, if a moderator flags a Global Rules detection as a false positive, that report (category, matched text, the flagged user's ID, and the reporting moderator's identity) is forwarded to a second, similarly operator-controlled channel. Neither is used to train machine learning or AI models вЂ” see Section 4.
...
### f) Billing Information (Paid Plans)
If you subscribe to a paid plan, payment is handled by a third-party payment processor. **We do not receive or store your full card number, CVC, or bank credentials** вЂ” those go directly to the processor. We retain only what we need to administer your subscription: which Discord server the plan applies to, the plan tier, its status and renewal date, and the processor's transaction reference. We may also receive limited billing details from the processor, such as the last four digits of a card, its expiry, and the billing country, for invoicing and fraud prevention. Where required by tax or accounting law, invoice records are kept for the statutory retention period.
...
- **Legitimate interest** вЂ” operating and securing the Service, and protecting server communities from fraud and abuse.
- **Consent** вЂ” where you have taken an affirmative action to verify through our dashboard.
- **Legal obligation** вЂ” where processing is required to comply with applicable law.
...
- **ohShield's own operator** (not an external third party), for confirmed scam-report bans and Global Rules false-positive flags вЂ” see Section 1(c). This is a cross-server record used only to review detection accuracy and is never used to train machine learning or AI models, sold, or disclosed to anyone outside the operator.
- **Payment processors**, if you subscribe to a paid plan, to take payment and handle billing, refunds, and chargebacks вЂ” see Section 1(f). They act as independent controllers of the card data you give them, under their own privacy policies.
...
- IP addresses collected for anti-abuse verification throttling (В§1b) are automatically deleted once the relevant rolling time window elapses.
- Images copied to a moderation channel from flagged messages (В§1c) are automatically deleted after a configurable retention period (24 hours by default; a server owner may configure a longer period).
- IP addresses logged by the internal Global Rules admin console (В§1e) are retained indefinitely as part of a security audit trail.
- Confirmed scam-report and false-positive records forwarded to the operator (В§1c, В§4) are retained indefinitely as a cross-server detection-accuracy record.
- If ohShield is removed from a server, associated server-specific data is retained until the server owner or an authorized admin requests its deletion by contacting us (see Section 11), except where retention is required for legal or security purposes. We do not currently run an automatic deletion schedule after removal вЂ” deletion is handled on request.
