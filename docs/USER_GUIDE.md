...
[How rules are made](HOW_RULES_ARE_MADE.md) for the full process, the
...
**Rate limiting:** reports are capped by a per-guild cooldown, a per-user cooldown, and a per-user hourly limit (all configurable, see [Scam Report Limits & Roles](#4-scam-report-limits--roles)) — trusted roles can be made immune to all three.
...
ohShield monitors your Discord AutoMod notifications channel and forwards all alerts to your Alerts channel.
...
Command `%check [duration]` or `/check` (admin only) runs a full channel-history scan against Global Rules only, over an admin-chosen history window. Gated by a global cooldown (shared by every server, default 30 days). Every match is actioned the same as a live detection — the offending message is deleted and the category's configured penalty (timeout/kick/ban) is applied — and posted to the scam alert channel as a Full Check Detection alert.
...
Optional verification challenges for new members. Enable any combination of four modes — stack them for a harder gate, or run just one:

1. Emoji — Select the correct emoji from a grid (configure: number of rounds)
2. CAPTCHA — Solve a math, text, or logic puzzle in an image (configure: number of tasks; puzzle type mix and per-type difficulty are set bot-wide from the superadmin console)
3. Odd-One-Out — A grid of near-identical rendered glyphs where exactly one is rotated, mirrored, or carries a subtle local edit; the member taps its number (configure: rounds, grid size 2x2–5x5, and puzzle style — `transform`, `micro`, or `both`)
4. Game2048 — Play 2048 up to a target score (configure: target score)
...
**Why 2048 stops bot farms.** The board lives on the server, not in the page — every move is validated against an authoritative engine, so a client can't submit a finished game it never played. Entry links are one-time and bound to the member who clicked Verify, and each move carries a short-lived nonce, so a captured request sequence can't be replayed later. On top of that, the server scores *how* the game was played: sustained sub-human move pacing, metronomic timing with implausibly low variance, bursts beyond real key-mashing, and near-perfect agreement with the engine's own best-move ranking all fail the session. The thresholds were tuned against real jittered human play so ordinary players pass. It is not unbreakable — a script that deliberately paces itself like a human for a whole session can still get through — but it turns a free, instant, parallelizable signup into a slow, per-account, human-timed one, which is the economics a bot farm runs on.
...
### 1. Configure the Alerts Channel

```
%set alerts #channel-name
```

**What it does:** One channel receives everything — Image Drop Detection triggers, scam reports, Global Rules detections, AutoMod forwards, verification events, bans, and timeouts — each with a timestamp and the action buttons moderators need.
...
To clear it again: `%set alerts remove`.

**Tip:** Create a private channel (viewable only by moderators) for this to keep security actions confidential. The onboarding wizard creates one for you on first run.
...
**What it does:** This is the channel where Discord's native AutoMod posts its notifications. ohShield monitors this channel and forwards alerts to your Alerts channel.
...
**Note:** You must enable AutoMod in your server's Safety Center first, and it must post notifications to a channel. Then tell ohShield where that channel is.

### 3. Whitelist Roles for Scam Reporting
...
### 5. Image Drop Detection Configuration
...
- [ ] `%set alerts #channel` — Configure the Alerts channel
...
```
%setverify enable <emoji|captcha|game2048|oddoneout>
%setverify disable <emoji|captcha|game2048|oddoneout>
...
%setverify oddrounds <n>       (odd-one-out mode)
%setverify oddgrid <2-5>       (odd-one-out grid size, n x n)
%setverify oddmode <transform|micro|both>  (odd-one-out puzzle style)
...
- `captcha` — Solve a math, text, or logic puzzle (requires tasks parameter)
- `oddoneout` — Spot the altered glyph in a grid (configure with `oddrounds`, `oddgrid`, `oddmode`)
...
| Set Alerts Channel | `%set alerts #channel` | Configure the single channel every security alert posts to |
...
2. Event is logged to the Alerts channel with timestamp, user, number of images, and moderator name
...
**Configuration Checklist:**
- [ ] `%set alerts #channel` — Configure the Alerts channel
- [ ] `%set alerts #channel` — Configure the Alerts channel (reports post here)
...
3. When it detects a notification, it forwards the alert to your Alerts channel
...
- [ ] `%set alerts #channel` — Configure the Alerts channel for forwarded alerts
...
3. Alert is logged to the Alerts channel with details of what was matched, with **Ignore** and **Report False Positive** buttons for moderators — the latter forwards the detection to the bot operator for accuracy review, and the text it wrongly flagged becomes a permanent regression test that every future rule is checked against (see [How rules are made](HOW_RULES_ARE_MADE.md))
...
4. Each matching message or member is posted to the scam alert channel in the same format, with the same action buttons (Ban, Dismiss, Report False Positive), as a live Global Rules detection — messages are never deleted or banned automatically (that history is stale by the time the check finds it), but a username impersonation match is still someone impersonating right now, so it gets the category's configured penalty (timeout by default) applied immediately, exactly as a live join/nickname/username-change detection would; either way, these finds count toward your Global Rules statistics the same way a live catch would
5. The notice is edited with a summary and a jump link to the results once the check finishes

**Configuration Checklist:**
- [ ] `%set alerts #channel` — Configure the Alerts channel (results are posted here)
...
- CAPTCHA — Solve a math, text, or logic puzzle in an image (configure: number of tasks; puzzle type mix and per-type difficulty are set bot-wide from the superadmin console)
...
ohShield stores:
- **Per-server settings** (Alerts channel, scam roles, thresholds, etc.)
...
If you see a formatted embed with your server's settings, the bot is working and protecting your server. Check that critical settings like "Alerts Channel" are configured and you're good to go.
...
- **Discord Server:** [discord.gg/a2CrKgjcSn](https://discord.gg/a2CrKgjcSn)
- **X:** [@ohShieldApp](https://x.com/ohShieldApp)
- **Email:** [yes@ohshield.com](mailto:yes@ohshield.com)
- **Issues & feedback:** [GitHub Issues](https://github.com/noname9006/ohShield/issues)
